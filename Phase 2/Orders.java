...
public class Orders {
    ...
    // Create Order - O(log n) for order insertion
    public Orders createOrder(int customerId, AVL_Tree<Integer, Products> productsList) {
        try {
            ...
            while (checkOrderID(oid)) { // O(log n)
            ...
            while (answer == 'y' || answer == 'Y') {
                ...
                while (!productsList.findKey(pid)) { // O(log n)
                ...
                if (product.getStock() == 0) {
                    ...
                    productsList.update(pid, product); // O(log n)
            ...
            orders.insert(oid, newOrder); // O(log n)
    ...
    public int cancelOrder(int id) {
        try {
            ...
            if (orders.findKey(id)) { // O(log n)
                ...
                orders.update(id, order); // O(log n)
    ...
    public boolean UpdateOrder(int orderID) {
        try {
            ...
            if (orders.findKey(orderID)) { // O(log n)
                ...
                orders.update(orderID, current); // O(log n)
    ...
    // NEW: Get orders between two dates - O(n) with in-order traversal
    ...
    // Check if order ID exists - O(log n)
    public boolean checkOrderID(int oid) {
        try {
            return orders.findKey(oid); // O(log n)
        } catch (Exception e) {
    ...
    // Load orders from file - O(n log n)
    public Orders(String fileName) {
        try {
            ...
            while (reader.hasNext()) {
                ...
                orders.insert(orderId, o); // O(log n)
                count++;
            }
