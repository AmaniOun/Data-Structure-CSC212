    ...
    public Orders createOrder(int customerId, LinkedList<Products> productsList) {
        try {
            ...
            while (answer == 'y' || answer == 'Y') {
                ...
                while (!checkProductExists(pid, productsList)) {
                ...
                for (int i = 0; i < productsList.size(); i++) {
                    if (productsList.retrieve().getProductId() == pid) {
                        if (productsList.retrieve().getStock() == 0) {
                            System.out.println("Product out of stock, try another time");
                        } else {
                            productsList.retrieve().setStock(productsList.retrieve().getStock() - 1);
                            System.out.println("Product added to order");
                            
                            newOrder.addProduct(productsList.retrieve().getProductId());
                            totalPrice += productsList.retrieve().getProductPrice();
                        }
                        break;
                    }
